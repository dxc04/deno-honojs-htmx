...
app.get("/", async (c: Context) => {
  ...
  const template = await tmpl.renderSessionApp(user, "profile", { data: { first_name: user.first_name, last_name: user.last_name, email: user.email } });
  ...
  zValidator(
    ...
    async (result, c: Context) => {
      ...
      if (result.success) {
        ...
        await tmpl.renderAlertSuccess("Profile updated successfully!", c.res.headers);
      }
  ...
  zValidator(
    ...
    async (result, c: Context) => {
      ...
      if (result.success) {
        ...
        await tmpl.renderAlertSuccess("User password updated successfully!", c.res.headers);
      }
