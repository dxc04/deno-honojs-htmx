...
export const renderSessionApp = async (user: User, page: string = "index", params: {[key: string]: unknown} = {}) => {
...
export const renderFormPartial = async (page: string, params: {[key: string]: unknown} = {}) => {
...
const renderAlert = async (message: string, type: string, iconDraw: string, headers: Headers) => {
  await headers.append("HX-Trigger", '{"show-alert":{"type" : "' + type + '", "message" : "' + message + '", "iconDraw": "' + iconDraw + '"}}');
};

export const renderAlertSuccess = async (message: string, headers: Headers) => {
  await renderAlert(message, "success", "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z", headers);
}
...
export const renderAlertError = async (message: string, headers: Headers) => {
  await renderAlert(message, "error", "M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z", headers);
}

export const renderAlertInfo = async (message: string, headers: Headers) => {
  await renderAlert(message, "info", "M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z", headers);
}